...
#else // WIN32
		 _tzset();
		 // mktime adjusts &ts to GMTIME according to local TZ.
		 // as &ts IS in GMTIME already, we have to shift it back...
		 _timestamp = mktime(&ts) - _timezone;
#endif // WIN32
	       return;
	  }
     }
...
Message::Message(const std::string& jid, const std::string& body, Message::Type mtype , bool escaped)
     : Packet("message")
{
     setTo(jid);
     if (!body.empty())
     _base.addElement("body", body , escaped);
     _timestamp = time(0);
     setType(mtype);
}

void Message::setBody(const std::string& body , bool escaped)
{
     ...
     if (body_tag)
     {
	  ...
	  for (; it != body_tag->end(); ++it )
	  {
	       if ((*it)->getType() != Node::ntCDATA)
		    continue;
	       (static_cast<CDATA*>(*it))->setText(body.c_str(), body.length() , escaped);
          }
     }
     else
	  _base.addElement("body", body , escaped);
